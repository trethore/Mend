...
fn build_clean_maps(
    source_lines: &[String],
) -> (Vec<(usize, String)>, HashMap<String, Vec<usize>>) {
...
fn test_strict_patch_succeeds() {
    ...
    let (clean_map, index_map) = build_clean_maps(&original_lines);
    let matches: Vec<HunkMatch> =
        patcher::find_hunk_location(&original_lines, &clean_map, &index_map, hunk, 0, false, 0.7);
...
fn test_fuzzy_patch_succeeds_when_strict_fails() {
    ...
    let (clean_map, index_map) = build_clean_maps(&original_lines);
    let matches: Vec<HunkMatch> =
        patcher::find_hunk_location(&original_lines, &clean_map, &index_map, hunk, 1, false, 0.7);

    // ASSERT: Check that we found a good match.
    assert_eq!(matches.len(), 1);
    assert!(matches[0].score >= 0.9 && matches[0].score <= 1.0);
