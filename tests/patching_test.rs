...
fn test_strict_patch_succeeds() {
    ...
    let matches: Vec<HunkMatch> = patcher::find_hunk_location(&original_lines, hunk, 0, false, 0.7);
...
fn test_fuzzy_patch_succeeds_when_strict_fails() {
    ...
    let matches: Vec<HunkMatch> = patcher::find_hunk_location(&original_lines, hunk, 1, false, 0.7);

    // ASSERT: Check that we found a good match.
    assert_eq!(matches.len(), 1);
    assert_eq!(matches[0].score, 0.9); // Level 1 match score
